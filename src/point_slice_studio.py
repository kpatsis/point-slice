...
def ask_user_input():
    folder = input(
        "📂 Drag and drop τον φάκελο με τα DXF slices και πάτα Enter: "
    ).strip('"')
    floor_info = input(
        "🏢 Δώσε ορόφους και slices (π.χ. Υπόγειο 01-03, Α Όροφος 04-06): "
    ).strip()
...
def create_block(
    doc, name, points, layer, color, z_avg=None, floor=None, text_pos=None
):
    ...
    if z_avg is not None and floor and text_pos:
        text = blk.add_text(
            f"Z: {z_avg:.2f} | {floor}",
            dxfattribs={"height": 0.7, "layer": layer, "color": color},
        )
...
def main():
    ...
    for i, (filename, slice_type, z_avg, sample, floor) in enumerate(slice_data):
        ...
        if slice_type == "XY":
            color = assign_color(
                xy_slices.index((filename, slice_type, z_avg, sample, floor)),
                len(xy_slices),
            )
        elif slice_type == "XZ":
            color = 6  # magenta
        ...
        blk = create_block(
            ...
            layer,
            color,
            z_avg if slice_type == "XY" else None,
            floor,
            text_pos,
        )
